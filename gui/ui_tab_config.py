...
class ConfigUI(AbstractComponentUI):
    ...
    def on_show(self, button_text):
        ...
        return gr.update(type="password"), gr.update(value=I18n.t("show"))
    ...
    def back_to_normal(self):
        ...
        time.sleep(3)
        return gr.update(value=I18n.t("save"))

    def create_ui(self):
        '''Create the config UI'''
        with gr.Tab(I18n.t("config_tab")) as config_ui:
            with gr.Row():
                with gr.Column():
                    with gr.Row():
                        openai_textbox = gr.Textbox(value=self.api_key_manager.get_api_key("OPENAI_API_KEY"), label=I18n.t("openai_api_key"), show_label=True, interactive=True, show_copy_button=True, type="password", scale=40)
                        show_openai_key = gr.Button(I18n.t("show"), size="sm", scale=1)
                        show_openai_key.click(self.on_show, [show_openai_key], [openai_textbox, show_openai_key])
                    with gr.Row():
                        eleven_labs_textbox = gr.Textbox(value=self.api_key_manager.get_api_key("ELEVENLABS_API_KEY"), label=I18n.t("elevenlabs_api_key"), show_label=True, interactive=True, show_copy_button=True, type="password", scale=40)
                        eleven_characters_remaining = gr.Textbox(value=self.get_eleven_remaining(), label=I18n.t("characters_remaining"), show_label=True, interactive=False, type="text", scale=40)
                        show_eleven_key = gr.Button(I18n.t("show"), size="sm", scale=1)
                        show_eleven_key.click(self.on_show, [show_eleven_key], [eleven_labs_textbox, show_eleven_key])
                    with gr.Row():
                        pexels_textbox = gr.Textbox(value=self.api_key_manager.get_api_key("PEXELS_API_KEY"), label=I18n.t("pexels_api_key"), show_label=True, interactive=True, show_copy_button=True, type="password", scale=40)
                        show_pexels_key = gr.Button(I18n.t("show"), size="sm", scale=1)
                        show_pexels_key.click(self.on_show, [show_pexels_key], [pexels_textbox, show_pexels_key])
                    with gr.Row():
                        gemini_textbox = gr.Textbox(value=self.api_key_manager.get_api_key("GEMINI_API_KEY"), label=I18n.t("gemini_api_key"), show_label=True, interactive=True, show_copy_button=True, type="password", scale=40)
                        show_gemini_key = gr.Button(I18n.t("show"), size="sm", scale=1)
                        show_gemini_key.click(self.on_show, [show_gemini_key], [gemini_textbox, show_gemini_key])

                    save_button = gr.Button(I18n.t("save"), size="sm", scale=1)
                    ...
                    save_button.click(lambda _: gr.update(value=I18n.t("keys_saved")), [], [save_button])
